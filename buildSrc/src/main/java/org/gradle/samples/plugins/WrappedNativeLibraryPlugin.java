...
public class WrappedNativeLibraryPlugin implements Plugin<Project> {
    ...
    @Inject
    public WrappedNativeLibraryPlugin(ImmutableAttributesFactory attributesFactory, TargetMachineFactory targetFactory) {
        this.attributesFactory = attributesFactory;
        this.targetMachineFactory = targetFactory;
    }
    ...
    public void apply(final Project project) {
        ...
        TargetMachine target = ((DefaultTargetMachineFactory) targetMachineFactory).host();
        ...
        Configuration linkDebug = project.getConfigurations().create("linkDebug", it -> {
            ...
            it.getAttributes().attribute(CppBinary.DEBUGGABLE_ATTRIBUTE, true);
            it.getAttributes().attribute(CppBinary.OPTIMIZED_ATTRIBUTE, false);
            it.getAttributes().attribute(OperatingSystemFamily.OPERATING_SYSTEM_ATTRIBUTE, target.getOperatingSystemFamily());
            it.getAttributes().attribute(MachineArchitecture.ARCHITECTURE_ATTRIBUTE, target.getArchitecture());
        });
